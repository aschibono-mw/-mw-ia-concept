...
export const CreateDashboardDrawer = ({ open, onClose }: CreateDashboardDrawerProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex">
      ...
      {/* Drawer Content */}
      <div className="fixed right-0 top-0 bottom-0 w-[80%] bg-card shadow-2xl animate-in slide-in-from-right duration-300 flex flex-col">
        {/* Header */}
        <div className="flex items-start justify-between p-6 pb-0">
          <div>
            <h2 className="text-2xl font-semibold text-foreground">Create a Dashboard</h2>
        ...
        <div className="px-6 pt-6">
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            <TabsList className="h-auto p-0 bg-transparent border-b border-border rounded-none w-full justify-start gap-6">
              <TabsTrigger 
                value="templates" 
                className="px-0 pb-3 -mb-px rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:shadow-none text-muted-foreground data-[state=active]:text-foreground font-medium"
              >
                Dashboard Templates
              </TabsTrigger>
              <TabsTrigger 
                value="intelligence" 
                className="px-0 pb-3 -mb-px rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:shadow-none text-muted-foreground data-[state=active]:text-foreground font-medium flex items-center gap-2"
              >
            ...
            <ScrollArea className="h-[calc(100vh-180px)] -mx-6 px-6">
                  ...
                  <div key={category.id}>
                    ...
                    <div className="grid grid-cols-4 gap-4">
                      {category.templates.map((template, index) => (
                        <div 
                          key={index} 
                          className="group border border-border rounded-xl p-5 hover:border-primary hover:shadow-md cursor-pointer transition-all bg-background"
                        >
                          ...
                          <Button variant="link" className="p-0 h-auto text-foreground underline hover:text-primary font-medium">
                            Create →
                          </Button>
                        </div>
                      ))}
              ...
              <TabsContent value="intelligence" className="mt-6 space-y-6">
                ...
                <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary/10 via-primary/5 to-background border-2 border-primary/30 p-8">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    {intelligenceProducts.filter(p => !p.featured).map((product, index) => (
                      <div 
                        key={index} 
                        className="group border border-border rounded-xl p-5 hover:border-primary/50 hover:shadow-md cursor-pointer transition-all bg-background relative overflow-hidden"
                      >
